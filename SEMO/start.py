   ...
   try:
    ...
    if message.new_chat_members[0].username == "JOO_B_R_Z":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "جـو")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطور جـو للشات  💎 .\n♪ مرحبا بك : @JOO_B_R_Z  💎 .**")
    dev = await get_dev(bot_username)
...
"مره واحد اسمو جـو  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
"لا جـو على الدوق فيما يحب أو بكره",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝙎𝙊𝙐𝙍𝘾𝙀 𝙏𝙐𝙇𝙈𝘽𝙍](t.me/TU_LM_BR) 💎 .
么  [𝒅𝒆𝒗: 𝑎𝑐𝑢𝑢𝑛𝑡𝑒𝑦](t.me/JOO_B_R_Z) 💎 .
╰──── • ◈ • ────╯
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("SeMo ElKbEr 🥷", user_id=f"𝑦𝑜𝑢𝑠𝑠𝑒𝑓 𝑏𝑟𝑎𝑧𝑖𝑙𝑖𝑎𝑛")
                        ],
                        [
...
@Client.on_message(filters.command(["المطور جـو","جـو","المبرمج"], ""))
...
@Client.on_message(filters.command(["الجـو","المطور جـو ","المطور جـو"], ""))
...
async def tiillli(client, message):
  if message.from_user.username in ["JOO_B_R_Z"]:
   ...
   async for msg in user.get_chat_history("JO_BRZ"):
       if not msg.text in tyet:
         tyet.append(msg.text)
   if message.from_user.username == "JOO_B_R_Z":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور جـو  💎 .**")
   else:
...
async def tiillllli(client, message):
 if message.from_user.username in ["JOO_B_R_Z"]:
   ...
   bot_username = client.me.username
   user = await get_userbot(bot_username)
   async for msg in user.get_chat_history("GOLDN_BR"):
...
async def sssora(client, message):
  ...
  if len(lisetanme) == 0:
     ...
     async for msg in user.get_chat_history("BPHEE"):
      if msg.media:
        lisetanme.append(msg)
  phot = random.choice(lisetanme)
  photo = f"https://t.me/BPHEE/{phot.id}"
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @TU_LM_BR  💎 .**")
...
async def soora4(client, message):
  ...
  if len(lisetbnat) == 0:
   user = await get_userbot(client.me.username)	
   async for msg in user.get_chat_history("BRKOQ"):
      if msg.media:
        lisetbnat.append(msg)
  phot = random.choice(lisetbnat)
  photo = f"https://t.me/BRKOQ/{phot.id}"
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @TU_LM_BR  💎 .**") 
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @TU_LM_BR  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @TU_LM_BR  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in ["JOO_B_R_Z"]:
         await message.reply_text("**♪ رتبتك هي : المطور جـو  💎 .**")
         return
     if userr.username in ["JOO_B_R_Z"]:
         await message.reply_text("**♪ رتبتك هي : المطور جـو 💎 .**")
         return
